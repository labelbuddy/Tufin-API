...
|------|-------------------|
| All rules on a device | `device.name='123'` |
...
| Device + stale | `device.name='123' and timeLastHit before 182 days ago` |
...
```graphql
query {
  rules(filter: "device.name='42' and timeLastHit before 182 days ago") {
    count
  }
}
```
...
query {
  rules(
    filter: "device.name='42' and timeLastHit before 182 days ago"
    first: 500
    offset: 0
  ) {
    count
